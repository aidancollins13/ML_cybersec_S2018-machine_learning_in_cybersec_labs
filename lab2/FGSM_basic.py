...
if(len(sys.argv) < 2):
    print("USAGE: %s <step_size>" % sys.argv)
    sys.exit(1)
...
def non_targeted(x_in, y_in, e):
    ...
    x_prime = (x + (e/256) * tf.sign(cost_grad[0]))/256
    xn = x_prime.eval({x: x_in, y: y_in})
    print("Accuracy: ", accuracy.eval({x:xn, y:y_in}))
    print()
    for i in range(xn.shape[0]):
        name = "basic_FGSM_untargeted/{}_{}.png".format(str(e),str(i))
        plt.imsave(name,xn[i].reshape(28,28))

def targeted_attack(x_in,y_in,e):
    ...
    print()
    for i in range(xn.shape[0]):
        name = "basic_FGSM_targeted/{}_{}.png".format(str(e),str(i))
        plt.imsave(name,xn[i].reshape(28,28))
...
with tf.Session() as sess:
    ...
    targeted_attack(mnist.test.images[:3000], mnist.test.labels[:3000], int(sys.argv[1]))
