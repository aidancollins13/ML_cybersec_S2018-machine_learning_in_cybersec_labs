...
print("Retraining witn a step size of %d" % int(sys.argv[1]))
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            xn = x_prime.eval({x: batch_xs, y: batch_ys})
            xn = np.vstack((batch_xs, xn))
            yn = np.vstack((batch_ys, batch_ys))
            # Fit training using batch data
            _, c = sess.run([optimizer, cost], feed_dict={x: xn,
                                                       y: yn})

#             print(__w)

            # Compute average loss
            avg_cost += c / total_batch*2
